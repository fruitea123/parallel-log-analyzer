...
This project fits Category 1 because it centers on a parent-controlled worker pool, meaningful pipe-based message passing, and concurrent execution across three child processes. The implementation is intentionally simple. The parent creates two pipes per worker, forks exactly three children, assigns one initial file to each worker, and then uses `select()` to wait for whichever worker finishes first. When a result arrives, the parent either assigns that worker another file or sends a termination message if no files remain. This design allows the program to process multiple files concurrently without introducing shared-memory synchronization. The worker-count constant appears in `include/protocol.h` (line 6), while worker creation and task dispatch are implemented in `spawn_workers` in `src/main.c` (lines 131–155), `seed_initial_tasks` in `src/main.c` (lines 269–283), and `run_scheduler` in `src/main.c` (lines 286–380).
...
The worker calls `recv_task`, which calls `read_full` with `sizeof(task_msg_t)` in `src/ipc.c` (lines 59–67). Since both parent and worker are compiled from the same program and use the same struct definition from `include/protocol.h`, one complete task message consists of exactly `sizeof(task_msg_t)` bytes.
...
Qingyi Jiang led the overall design, implementation, testing, and documentation of the project. This included designing the parent–worker architecture, implementing the pipe-based communication protocol, building the scheduling logic in the parent controller, implementing the worker loop and log-analysis logic, adding robustness improvements such as aggregate summary and error-handling cleanup, validating the program in WSL, and writing the final report.

Mengxuan Guo contributed substantially to the implementation and validation of the project. This included helping develop and review the core program logic, assisting with testing and debugging, checking the behavior of the scheduler and worker communication, and contributing to the preparation and revision of the final reprt and demo.

Leyi Wu contributed to the documentation materials for the project, including preparing the final architecture diagram for the report based on the agreed system design and draft sketch, and reviewing the final submission materials.
